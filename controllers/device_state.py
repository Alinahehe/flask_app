...
class DeviceState(ControllerBase):
    def get(self):
        ...
        return {
                'error':1,
                'messege': 'Неверный идентификатор устройства',
                'data': None,
                }, 200
    def post(self):
        parser = DeviceState.parser.copy()
        ...
        if data['type']=='sensor':
            return {
                'error':2,
                'messege': 'Невозможно изменить состояние данного устройства',
                'data': None,
                }, 200            
        ...
        if data ['status']!=args['value']:
            return {
                'error':3,
                'messege': 'Не удалось изменить состояние данного устройства',
                'data': None
                }, 200
        return {
                'error':0,
